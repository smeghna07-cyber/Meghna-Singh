...
export interface VerticalBarsNoiseProps {
...
export const VerticalBarsNoise = ({
  ...
  const animate = useCallback(() => {
    ...
    const canvasWidth = canvas.clientWidth || canvas.width / dprRef.current;
    const canvasHeight = canvas.clientHeight || canvas.height / dprRef.current;

    const numLines = Math.max(10, Math.floor(canvasHeight / 14));
    ...
    for (let i = 0; i < numLines; i++) {
      ...
      const lineAlpha = Math.max(0.2, 0.2 + mouseInfluence * 0.6);
      ...
      for (let x = 0; x < canvasWidth; x += 10) {
        ...
        if (noiseVal > threshold) {
          const barWidth = 3 + noiseVal * 8 + totalInfluence * 4;
          const barHeight = 2 + noiseVal * 3 + totalInfluence * 3;

          const baseAnimation =
            Math.sin(timeRef.current + y * 0.0375) * 16 * noiseVal;
          ...
          const intensity = Math.min(
            1,
            Math.max(0.6, 0.6 + totalInfluence * 0.4)
          );
